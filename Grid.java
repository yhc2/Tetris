...
public class Grid {
	...
	private void remove(int r) { //r ->the row which requires to be removed
		// remove the row by making it white
		for (int col = 0; col < WIDTH ; col++) {
			set(r,col,EMPTY);
		}
	...
	public void checkRows() {
		...
		for (row = 0; row < HEIGHT; row++) {
			for (col = 0; col < WIDTH; col++) {
				if(!isSet(row,col)) 
					break; //stop counting column when there's empty
				}
			if (col == WIDTH){ //the entire row is filled
				remove(row);
			}
		}
	}
